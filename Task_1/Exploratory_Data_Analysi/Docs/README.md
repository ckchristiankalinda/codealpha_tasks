...
4. **Launch the notebook**
   ```bash
   jupyter notebook notebooks/Superstore_Sales_Performance_Analysis.ipynb
   ```

   Alternatively, open `reports/Superstore_Sales_Performance_Analysis.html` directly in any browser — no installation required.

## 👤 About the Author

**Mutia Kalinda Christian (CK)**
Data Analyst | Data Science with AI (in progress) | Kampala, Uganda 🇨🇩🇺🇬

Focused on turning data into clear, actionable business decisions — with a particular interest in data analytics and BI roles supporting humanitarian and public health work in the Democratic Republic of Congo.
...
This project is licensed under the [MIT License](LICENSE) — feel free to use, adapt, and build on it, with attribution.
